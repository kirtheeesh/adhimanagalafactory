            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-100">
                <div className="space-y-1">
                    <div className="flex items-center gap-2 text-[#e85c24] text-[10px] lg:text-xs font-black uppercase tracking-[0.2em] mb-1 lg:mb-2">
                        <Truck size={14} className="lg:w-3.5 lg:h-3.5" /> Logistics Records
                    </div>
                    <h1 className="text-2xl lg:text-4xl font-black text-slate-900 tracking-tight">Dispatch History</h1>
                    <p className="text-sm lg:text-base text-slate-500 font-medium">Review and download all dispatch reports and delivery notes.</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[700px] lg:min-w-full">
                        <thead>
                            <tr className="bg-slate-50/50 border-b border-slate-100">
                                <th className="px-6 lg:px-8 py-4 lg:py-6 text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest">Dispatch Details</th>
                                <th className="px-6 lg:px-8 py-4 lg:py-6 text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Batch</th>
                                ...
                                <th className="px-6 lg:px-8 py-4 lg:py-6 text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest">Dispatched By</th>
                            ...
                            {loading ? (
                                [1, 2, 3, 4, 5].map(i => (
                                    <tr key={i} className="animate-pulse">
                                        <td colSpan={5} className="px-6 lg:px-8 py-4 lg:py-6 h-20 bg-slate-50/50" />
                                    </tr>
                                ))
                            ) : error ? (
                                <tr>
                                ...
                                <tr>
                                    <td colSpan={5} className="px-8 py-20 text-center">
                                        ...
                                        <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest">No dispatch records found</p>
                                    </td>
                                </tr>
                            ) : (
                                    ...
                                    >
                                        <td className="px-6 lg:px-8 py-4 lg:py-6">
                                            <div className="flex items-center gap-3 lg:gap-4">
                                                ...
                                                <div>
                                                    <p className="text-xs lg:text-sm font-black text-slate-900 uppercase tracking-tight leading-none">DISPATCH #{item.id}</p>
                                                    <p className="text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1.5 leading-none">{new Date(item.dispatched_at).toLocaleString('en-GB', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</p>
                                                    <p className="text-[9px] lg:text-[10px] text-[#e85c24] font-black uppercase mt-1 leading-tight">{item.items_summary}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 lg:px-8 py-4 lg:py-6 text-center">
                                            <span className="text-xs lg:text-sm font-black text-slate-700 tracking-tight uppercase">{item.batch_number}</span>
                                        </td>
                                        <td className="px-6 lg:px-8 py-4 lg:py-6">
                                            <span className="text-[10px] lg:text-[11px] font-black text-slate-500 uppercase tracking-widest leading-tight block max-w-[150px] truncate">{item.customer_name}</span>
                                        </td>
                                        <td className="px-6 lg:px-8 py-4 lg:py-6">
                                            <span className="text-xs lg:text-sm font-black text-slate-700 uppercase tracking-tight">{item.dispatched_by_name}</span>
                                        </td>
