...
const Login: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const errorMessage = err instanceof Error ? err.message : 'Authentication sequence failed.';
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-[#f4f4f5] p-6 selection:bg-orange-100 selection:text-orange-900 overflow-hidden relative">
            ...
            <motion.div 
                ...
                className="w-full max-w-[1100px] grid grid-cols-1 lg:grid-cols-2 bg-white rounded-3xl lg:rounded-[3rem] shadow-2xl shadow-slate-200 overflow-hidden relative z-10 border border-slate-100"
            >
                ...
                >
                    ...
                    <div className="absolute inset-0 opacity-20">
                        <div className="absolute inset-0 bg-gradient-to-br from-[#e85c24] to-transparent" />
                ...
                <div className="p-8 sm:p-12 lg:p-20 flex flex-col justify-center bg-white">
                    <div className="mb-8 lg:mb-12">
                        <div className="lg:hidden flex items-center gap-2 mb-6 sm:mb-8">
                            ...
                            <span className="text-lg lg:text-xl font-black tracking-tighter uppercase italic">ADHMANGALAM</span>
                        </div>
                        <h1 className="text-2xl lg:text-3xl font-black text-slate-900 tracking-tight mb-1 lg:mb-2 uppercase">Command Access</h1>
                        <p className="text-slate-500 font-bold text-[10px] lg:text-sm uppercase tracking-widest leading-none">PROVISIONING GATEWAY</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6 lg:space-y-8">
                        <AnimatePresence>
                            {error && (
                                <motion.div 
                                    ...
                                    exit={{ opacity: 0, x: 20 }}
                                    className="p-4 bg-rose-50 text-rose-600 text-[10px] lg:text-xs font-black uppercase tracking-widest rounded-xl lg:rounded-2xl border border-rose-100 flex items-center gap-3 shadow-sm"
                                >
                                    <Fingerprint size={16} className="lg:w-[18px] lg:h-[18px]" />
                                    {error}
                                </motion.div>
                            )}
                        </AnimatePresence>
                        ...
                        <div className="space-y-4 lg:space-y-6">
                            <div className="group">
                                <label className="block text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2 lg:mb-3 ml-1 group-focus-within:text-[#e85c24] transition-colors">Credential Identification</label>
                                <div className="relative">
                                    <div className="absolute left-5 lg:left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#e85c24] transition-colors">
                                    ...
                                    <input 
                                        ...
                                        className="w-full pl-14 lg:pl-16 pr-6 lg:pr-8 py-4 lg:py-5 bg-slate-50 border-none rounded-2xl lg:rounded-3xl text-sm font-bold text-slate-900 focus:ring-4 focus:ring-[#e85c24]/10 outline-none transition-all placeholder:text-slate-300"
                            ...
                            <div className="group">
                                <label className="block text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2 lg:mb-3 ml-1 group-focus-within:text-[#e85c24] transition-colors">Access Protocol</label>
                                <div className="relative">
                                    <div className="absolute left-5 lg:left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#e85c24] transition-colors">
                        ...
                        <button 
                            type="submit"
                            disabled={isLoading}
                            className="w-full group relative bg-slate-900 text-white py-4 lg:py-5 rounded-2xl lg:rounded-3xl font-black text-[10px] lg:text-xs uppercase tracking-[0.3em] overflow-hidden transition-all active:scale-[0.98] disabled:opacity-50 disabled:active:scale-100 shadow-xl shadow-slate-200"
                        >
                            ...
                            <span className="relative z-10 flex items-center justify-center gap-2 lg:gap-3">
                                {isLoading ? (
                                    ...
                                    <>
                                        ...
                                        <ArrowRight size={16} className="lg:w-[18px] lg:h-[18px] group-hover:translate-x-2 transition-transform duration-500" />
                                    </>
                                )}
                            </span>
                        </button>
                    ...
                    <div className="mt-8 lg:mt-12 pt-6 lg:pt-8 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-2 text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            <Zap size={14} className="text-[#e85c24]" /> Authorized Access
                        </div>
                        <a href="#" className="text-[9px] lg:text-[10px] font-black text-[#e85c24] uppercase tracking-widest hover:underline decoration-2 underline-offset-4 transition-all">
                            Forgot Protocol?
                        </a>
                    </div>
                </div>
