...
const PurchaseApprovals: React.FC = () => {
    ...
    const [trashSection, setTrashSection] = useState<'purchase-request' | 'purchase-order'>('purchase-order');
    ...
    const handleDelete = async (id: number) => {
        if (!window.confirm('Are you sure you want to move this purchase order to trash? Related accounts entries will be cleaned up.')) return;
        try {
        ...
        <div className="max-w-7xl mx-auto space-y-6 lg:space-y-10 animate-in fade-in duration-700 relative p-4 lg:p-0">
            <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 pb-6 border-b border-slate-100">
                <div className="space-y-1">
                    ...
                    <h1 className="text-2xl lg:text-4xl font-black text-slate-900 tracking-tight">Purchase Order Approvals</h1>
                    <p className="text-sm lg:text-base text-slate-500 font-medium">Verify and authorize final purchase orders from Accounts.</p>
                </div>
                <div className="flex gap-3">
                    <button 
                        onClick={() => { setTrashSection('purchase-request'); setIsTrashOpen(true); }}
                    ...
                    <button 
                        onClick={() => { setTrashSection('purchase-order'); setIsTrashOpen(true); }}
                        className="px-6 py-3 bg-slate-100 text-slate-600 rounded-xl text-[11px] lg:text-sm font-bold shadow-sm uppercase tracking-widest hover:bg-slate-200 transition-all flex items-center gap-2"
                    >
                        <Trash2 size={16} />
                        Order Trash
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
                <AnimatePresence mode="popLayout">
                    {loading ? (
                        [1, 2, 3].map(i => (
                            <div key={i} className="h-[300px] lg:h-[350px] bg-slate-100 rounded-3xl lg:rounded-[2.5rem] animate-pulse" />
                        ))
                    ) : error ? (
                        <div className="col-span-full py-12 lg:py-20 text-center">
                            <div className="p-6 lg:p-8 bg-rose-50 rounded-2xl lg:rounded-3xl border border-rose-100 max-w-md mx-auto">
                                ...
                                <button onClick={fetchOrders} className="w-full lg:w-auto px-6 py-3 bg-slate-900 text-white rounded-xl text-sm font-bold shadow-xl">Retry</button>
                            </div>
                        </div>
                        ...
                        filteredOrders.map((order, idx) => (
                            <motion.div 
                                ...
                                key={order.id}
                                className="glass p-6 lg:p-8 rounded-3xl lg:rounded-[2.5rem] border border-slate-100 shadow-soft group relative flex flex-col"
                            >
                                <div className="absolute top-0 right-0 p-4 lg:p-6 flex gap-2">
                                    <button 
                                        onClick={() => handleDelete(order.id)} 
                                        className="p-2.5 lg:p-3 bg-white text-slate-400 hover:text-rose-600 rounded-lg lg:rounded-xl shadow-soft transition-all active:scale-90 opacity-100 lg:opacity-0 group-hover:opacity-100"
                                    >
                                ...
                                <div className="flex-1 space-y-4 lg:space-y-6">
                                    ...
                                    <div>
                                        <h3 className="text-lg lg:text-xl font-black text-slate-900 tracking-tight mb-1 group-hover:text-primary transition-colors line-clamp-1 uppercase">
                                            {order.material_name}
                                        </h3>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 lg:gap-4">
                                        <div className="bg-slate-50 p-3 lg:p-4 rounded-xl lg:rounded-2xl">
                                            ...
                                            <div className="flex items-center gap-2 text-slate-900 font-black">
                                                ...
                                                <span className="text-base lg:text-lg">{order.purchased_quantity}</span>
                                            </div>
                                        </div>
                                        <div className="bg-primary/10 p-3 lg:p-4 rounded-xl lg:rounded-2xl">
                                            ...
                                            <div className="flex items-center gap-1 text-primary font-black">
                                                ...
                                                <span className="text-base lg:text-lg">{order.price}</span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="mt-6 lg:mt-8 space-y-2 lg:space-y-3">
                                    <button 
                                        ...
                                        className="w-full py-3 lg:py-3.5 bg-primary hover:opacity-90 text-white rounded-xl lg:rounded-2xl text-[10px] lg:text-xs font-black uppercase tracking-widest shadow-lg shadow-primary/20 transition-all active:scale-95"
                                    >
                                        Approve Purchase
                                    </button>
                                    <button 
                                        onClick={() => handleDownloadPDF(order.id)}
                                        className="w-full py-3 lg:py-3.5 bg-white border border-slate-100 text-slate-600 rounded-xl lg:rounded-2xl text-[10px] lg:text-xs font-black uppercase tracking-widest hover:bg-slate-50 transition-all active:scale-95"
                                    >
                                        View Invoice
                                    </button>
                                </div>
