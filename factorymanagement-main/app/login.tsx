...
export default function SecureFactoryPortal() {
  ...
  const [selectedDept, setSelectedDept] = useState('PRODUCTION HEAD');
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        const managementRoles = ['PRODUCTION HEAD', 'QUALITY', 'PACKING', 'SALES'];
        if (!managementRoles.includes(userRole)) {
          Alert.alert("Access Denied", "This app is for Management only.");
          setLoading(false);
          return;
        }
        ...
        // 3. SEPARATE DB LOGGING FOR HEADS
        ...
        switch (userRole) {
          ...
          default: router.replace('/phead'); break;
        }
      } else {
  ...
  return (
    <ImageBackground 
      ...
      imageStyle={{ width: '135%', left: '-15%', top: 50 }}
      resizeMode="cover"
    >
