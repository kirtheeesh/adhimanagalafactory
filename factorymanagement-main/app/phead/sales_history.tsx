...
export default function PHeadSalesHistoryScreen() {
  ...
  const renderInvoice = ({ item }: { item: any }) => (
    <View style={styles.card}>
      <View style={styles.cardInfo}>
        <Text style={styles.batchNum}>SALES ID: #{item.sales_request_id || item.id}</Text>
  ...
  const handleBack = () => {
    if (pathname.startsWith('/phead')) router.replace('/phead');
    else if (pathname.startsWith('/sales')) router.replace('/sales');
    else if (pathname.startsWith('/accounts')) router.replace('/accounts');
    else router.back();
  };
